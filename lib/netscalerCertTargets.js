                                    ...
                                    if(lbvserver_servicegroup_binding[netscalers[i]].hasOwnProperty(lbvserverkeys[j])) {
                                        ...
                                        if(servicegroup_servicegroupmember_binding[netscalers[i]].hasOwnProperty(servicegroupname)) {
                                            for(let k = 0; k < servicegroup_servicegroupmember_binding[netscalers[i]][servicegroupname].length; k++) {
                                                if(servicegroup_servicegroupmember_binding[netscalers[i]][servicegroupname][k].state == 'ENABLED' && servicegroup_servicegroupmember_binding[netscalers[i]][servicegroupname][k].svrstate != 'DOWN' && servicegroup_servicegroupmember_binding[netscalers[i]][servicegroupname][k].port.toString().indexOf('443') >= 0) {
            ...
            if(err) {
                ...
                if(resp.headers.hasOwnProperty('www-authenticate')) {
                    ...
                    if(resp.error) {
                        ...
                        callback("Unexpected response from " +  params.netscaler.host + ': ' + resp.body, false);
                    } else {
